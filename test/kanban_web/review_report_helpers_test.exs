defmodule KanbanWeb.ReviewReportHelpersTest do
  ...
  describe "has_explorer_result?/1 (W1958)" do
    test "true for a populated explorer_result map" do
      assert ReviewReportHelpers.has_explorer_result?(%{
               explorer_result: %{"dispatched" => true}
             })
    end

    test "false for an empty map, for nil, and for a task map with no explorer_result key" do
      refute ReviewReportHelpers.has_explorer_result?(%{explorer_result: %{}})
      refute ReviewReportHelpers.has_explorer_result?(%{explorer_result: nil})
      refute ReviewReportHelpers.has_explorer_result?(%{})
    end
    ...
    test "false for a non-map explorer_result" do
      refute ReviewReportHelpers.has_explorer_result?(%{explorer_result: "dispatched"})
      refute ReviewReportHelpers.has_explorer_result?(%{explorer_result: []})
    end
  end
  ...
  describe "explorer_panel_visible?/1 (W1958)" do
    test "true when explorer_result is populated and review_status is set" do
      assert ReviewReportHelpers.explorer_panel_visible?(%{
               explorer_result: %{"dispatched" => true},
    ...
    test "false for an open or in-progress task with no review_status" do
      for status <- [:open, :in_progress] do
        refute ReviewReportHelpers.explorer_panel_visible?(%{
                 explorer_result: %{"dispatched" => true},
                 review_status: nil,
                 status: status
               })
    ...
    test "false when explorer_result is missing entirely" do
      refute ReviewReportHelpers.explorer_panel_visible?(%{status: :completed})
      refute ReviewReportHelpers.explorer_panel_visible?(%{})
    end
    ...
    test "is not gated on needs_review or column name" do
      ...
      assert ReviewReportHelpers.explorer_panel_visible?(task)
    end
  end
  ...
  describe "skip_reason_label/1 (W1958)" do
    test "returns a non-empty label for each of the five enum values" do
      for reason <- [
        ...
        label = ReviewReportHelpers.skip_reason_label(reason)
    ...
    test "each of the five enum values maps to a distinct label" do
      labels =
        Enum.map(
          ...
          &ReviewReportHelpers.skip_reason_label/1
        )
    ...
    test "returns the raw string for an unrecognized value without raising" do
      assert ReviewReportHelpers.skip_reason_label("some_future_reason") == "some_future_reason"
      assert ReviewReportHelpers.skip_reason_label("") == ""
    end

    test "returns an empty string for a non-binary value without raising" do
      assert ReviewReportHelpers.skip_reason_label(nil) == ""
      assert ReviewReportHelpers.skip_reason_label(:self_reported_review) == ""
      assert ReviewReportHelpers.skip_reason_label(42) == ""
    end

    test "does not convert the incoming value to an atom" do
      before = :erlang.system_info(:atom_count)

      ReviewReportHelpers.skip_reason_label("definitely_not_an_existing_atom_w1958")
