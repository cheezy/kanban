defmodule Kanban.MetricsTest do
  ...
  describe "after_goal_adoption_7d/1 (W499)" do
    # Helper: mark a goal as having a real after_goal report at a given
    # offset (days_ago) from now. Mirrors the controller's report payload
    # shape (`exit_code`, `output`, `duration_ms`, `reported_at`) and
    # bypasses the full /complete + /after_goal flow so the tests stay
    # focused on the metric query.
    #
    # The standard Task changeset does NOT cast `after_goal_status` or
    # `after_goal_result` (only Kanban.Tasks.Goals writes them, via
    # `Ecto.Changeset.change/2` against the bare schema). The helper
    # mirrors that direct write so the test fixtures land in the same
    # row shape the query sees in production.
    defp ws_after_goal!(goal, days_ago, attrs \\ %{}) do
      ws_after_goal_at!(goal,
        seconds_offset: -days_ago * 86_400,
        attrs: attrs
      )
    end

    # Lower-level helper that takes a signed seconds offset on the
    # reported_at timestamp relative to `now`. Lets boundary tests place
    # rows at exactly window_start ± 1s without floating to day-granularity.
    defp ws_after_goal_at!(goal, opts) do
      seconds_offset = Keyword.fetch!(opts, :seconds_offset)
      ...
      result = Map.merge(base, Map.new(attrs, fn {k, v} -> {to_string(k), v} end))

      {:ok, g} =
        goal
        |> Ecto.Changeset.change(%{after_goal_status: :succeeded, after_goal_result: result})
        |> Kanban.Repo.update()

      g
    end

    test "returns 0 when scope yields no boards" do
      user = user_fixture()
      scope = Scope.for_user(user)
      assert Metrics.after_goal_adoption_7d(scope: scope) == 0
    end

    test "returns 0 when scope is nil" do
      assert Metrics.after_goal_adoption_7d(scope: nil) == 0
    end

    test "counts a single board with one real after_goal report inside the window",
         %{} do
      %{column: column, scope: scope} = ws_setup()
      column |> task_fixture(%{type: :goal}) |> ws_after_goal!(1)

      assert Metrics.after_goal_adoption_7d(scope: scope) == 1
    end

    test "counts a board exactly once when it has multiple after_goal reports",
         %{} do
      %{column: column, scope: scope} = ws_setup()

      # Three separate goals on the same board, each reporting in window.
      column |> task_fixture(%{type: :goal}) |> ws_after_goal!(1)
      column |> task_fixture(%{type: :goal}) |> ws_after_goal!(2)
      column |> task_fixture(%{type: :goal}) |> ws_after_goal!(3)

      assert Metrics.after_goal_adoption_7d(scope: scope) == 1
    end

    test "counts distinct boards across the user's workspace", %{} do
      user = user_fixture()
      board1 = board_fixture(user)
      board2 = board_fixture(user)
      col1 = column_fixture(board1)
      ...
      col1 |> task_fixture(%{type: :goal}) |> ws_after_goal!(1)
      col2 |> task_fixture(%{type: :goal}) |> ws_after_goal!(2)

      assert Metrics.after_goal_adoption_7d(scope: Scope.for_user(user)) == 2
    end

    test "excludes the back-compat grace-worker no-op report (pitfall)", %{} do
      %{column: column, scope: scope} = ws_setup()

      # Synthesized exactly the way Kanban.AfterGoal.GraceWorker writes
      # it — same `source` tag the metric query screens against.
      column
    ...
    test "7-day window boundary: includes >= -7d, excludes < -7d", %{} do
      %{column: column, scope: scope} = ws_setup()

      # Two goals straddling the trailing 7-day boundary by 1 second.
      # The query uses `>= now - 7 days`, so the +1s row must be counted
      # and the -1s row must be excluded.
      column
      |> task_fixture(%{type: :goal})
      |> ws_after_goal_at!(seconds_offset: -7 * 86_400 + 1)
    ...
    test "filters strictly to boards the scoped user belongs to", %{} do
      %{column: column, scope: scope} = ws_setup()
      column |> task_fixture(%{type: :goal}) |> ws_after_goal!(1)

      # A second user's board with its own after_goal report must not leak.
      other = user_fixture()
      other_board = board_fixture(other)
      other_col = column_fixture(other_board)
      other_col |> task_fixture(%{type: :goal}) |> ws_after_goal!(1)
      other_col |> task_fixture(%{type: :goal}) |> ws_after_goal!(2)

      assert Metrics.after_goal_adoption_7d(scope: scope) == 1
    end

    test "ignores work-typed tasks with after_goal_result (defensive)", %{} do
      %{column: column, scope: scope} = ws_setup()
      column |> task_fixture(%{type: :work}) |> ws_after_goal!(1)

      assert Metrics.after_goal_adoption_7d(scope: scope) == 0
    end

    test "ignores rows with after_goal_status != :succeeded", %{} do
      %{column: column, scope: scope} = ws_setup()
      ...
      # Emulates an in-flight :pending row: real report shape, but the
      # :succeeded flip hasn't happened yet. Same direct-write path the
      # production code uses (the default changeset does not cast these).
      {:ok, _} =
        goal
        |> Ecto.Changeset.change(%{
          after_goal_status: :pending,
          after_goal_result: %{
            "exit_code" => 1,
            "output" => "still trying",
            "duration_ms" => 50,
  ...
  describe "goal_to_done_latency_percentiles/1 (W500)" do
    # Helper: stage a completed goal with two children whose completion
    # times are spaced such that `max(child.completed_at) = goal.completed_at - latency_seconds`.
    # The goal's after_goal_result mirrors the controller's real-agent shape
    # (no `source` key) unless overridden via the `attrs` map.
    defp ws_goal_latency!(column, goal_completed_days_ago, latency_seconds, attrs \\ %{}) do
      ws_goal_latency_at!(column,
        seconds_offset: -goal_completed_days_ago * 86_400,
        latency_seconds: latency_seconds,
        attrs: attrs
      )
    end

    # Lower-level helper that takes a signed seconds offset for the goal's
    # completed_at relative to `now`. Lets boundary tests place rows at
    # exactly window_start ± 1s without floating to day-granularity.
    defp ws_goal_latency_at!(column, opts) do
      seconds_offset = Keyword.fetch!(opts, :seconds_offset)
      latency_seconds = Keyword.fetch!(opts, :latency_seconds)
      attrs = Keyword.get(opts, :attrs, %{})

      now = DateTime.utc_now() |> DateTime.truncate(:second)
      goal_completed_at = DateTime.add(now, seconds_offset, :second)
      final_child_completed_at = DateTime.add(goal_completed_at, -latency_seconds, :second)

      goal = task_fixture(column, %{type: :goal})

      seed_latency_children!(column, goal, final_child_completed_at)
      finalize_latency_goal!(goal, goal_completed_at, latency_seconds, attrs)
    end

    defp seed_latency_children!(column, goal, final_child_completed_at) do
      # Two children; only the most-recent matters for max(completed_at).
      child1 = task_fixture(column, %{parent_id: goal.id})
      child2 = task_fixture(column, %{parent_id: goal.id})

      Tasks.update_task(child1, %{
        claimed_at: DateTime.add(final_child_completed_at, -3600, :second),
        completed_at: DateTime.add(final_child_completed_at, -120, :second)
      })

      Tasks.update_task(child2, %{
        claimed_at: DateTime.add(final_child_completed_at, -3600, :second),
        completed_at: final_child_completed_at
      })
    end

    defp finalize_latency_goal!(goal, goal_completed_at, latency_seconds, attrs) do
      result = build_latency_result(goal_completed_at, latency_seconds, attrs)
    ...
    defp build_latency_result(goal_completed_at, latency_seconds, attrs) do
      base = %{
        "exit_code" => 0,
        "output" => "ok",
        "duration_ms" => latency_seconds * 1000,
        "reported_at" => DateTime.to_iso8601(goal_completed_at)
      }

      Map.merge(base, Map.new(attrs, fn {k, v} -> {to_string(k), v} end))
    end

    test "returns zero percentiles when scope yields no boards" do
      user = user_fixture()
      scope = Scope.for_user(user)

      assert Metrics.goal_to_done_latency_percentiles(scope: scope) ==
               %{p50_seconds: 0, p95_seconds: 0, sample_size: 0}
    end

    test "returns zero percentiles when scope is nil" do
      assert Metrics.goal_to_done_latency_percentiles(scope: nil) ==
               %{p50_seconds: 0, p95_seconds: 0, sample_size: 0}
    end

    test "returns zero percentiles when no eligible goals exist in the window", %{} do
      %{scope: scope} = ws_setup()
    ...
    test "computes p50 and p95 over a single sample", %{} do
      %{column: column, scope: scope} = ws_setup()
      ws_goal_latency!(column, 1, 30)

      result = Metrics.goal_to_done_latency_percentiles(scope: scope)
      assert result.sample_size == 1
      assert result.p50_seconds == 30
      assert result.p95_seconds == 30
    end

    test "computes p50 and p95 over a known sample using nearest-rank", %{} do
      %{column: column, scope: scope} = ws_setup()

      # 10 samples in seconds: 10, 20, 30, ..., 100.
      Enum.each(1..10, fn n -> ws_goal_latency!(column, 1, n * 10) end)

      result = Metrics.goal_to_done_latency_percentiles(scope: scope)
      assert result.sample_size == 10
      # Nearest-rank: p50 of [10,20,...,100] -> ceil(0.5*10)=5 -> sorted[4] = 50
      assert result.p50_seconds == 50
      # p95 -> ceil(0.95*10)=10 -> sorted[9] = 100
      assert result.p95_seconds == 100
    end

    test "excludes grace-worker-promoted goals from the sample (pitfall)", %{} do
      %{column: column, scope: scope} = ws_setup()

      # One real-agent goal with a meaningful latency.
      ws_goal_latency!(column, 1, 600)
      # Two grace-worker goals with near-zero latency that WOULD skew p95 down.
      ws_goal_latency!(column, 1, 1, %{"source" => "after_goal_grace_worker"})
      ws_goal_latency!(column, 1, 1, %{"source" => "after_goal_grace_worker"})

      result = Metrics.goal_to_done_latency_percentiles(scope: scope)
      assert result.sample_size == 1
      assert result.p50_seconds == 600
      assert result.p95_seconds == 600
    end

    test "14-day window boundary: includes >= -14d, excludes < -14d", %{} do
      %{column: column, scope: scope} = ws_setup()

      # Two rows straddling the boundary by 1 second on either side.
      # The query uses `>= now - 14 days`, so the +1s row must be included
      # and the -1s row must be excluded. A regression flipping >= to > or
      # drifting the window by ±12h would change at least one count below.
      ws_goal_latency_at!(column, seconds_offset: -14 * 86_400 + 1, latency_seconds: 100)
      ws_goal_latency_at!(column, seconds_offset: -14 * 86_400 - 1, latency_seconds: 999)

      result = Metrics.goal_to_done_latency_percentiles(scope: scope)
      assert result.sample_size == 1
      assert result.p50_seconds == 100
      assert result.p95_seconds == 100
    end

    test "filters strictly to the scoped user's boards", %{} do
      %{column: column, scope: scope} = ws_setup()
      ws_goal_latency!(column, 1, 50)

      other = user_fixture()
      other_board = board_fixture(other)
      other_col = column_fixture(other_board)
      ws_goal_latency!(other_col, 1, 999_999)
    ...
    test "ignores in-flight goals (after_goal_status != :succeeded)", %{} do
      %{column: column, scope: scope} = ws_setup()
      ...
      now = DateTime.utc_now() |> DateTime.truncate(:second)
      Tasks.update_task(child, %{completed_at: DateTime.add(now, -60, :second)})
      Tasks.update_task(goal, %{completed_at: now})
    ...
    test "ignores goals whose children have no completed_at (HAVING guard)", %{} do
      %{column: column, scope: scope} = ws_setup()

      goal = task_fixture(column, %{type: :goal})
      # Children created but never completed.
      _child1 = task_fixture(column, %{parent_id: goal.id})
      _child2 = task_fixture(column, %{parent_id: goal.id})

      now = DateTime.utc_now() |> DateTime.truncate(:second)
      {:ok, goal} = Tasks.update_task(goal, %{completed_at: now})
