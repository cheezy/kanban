defmodule KanbanWeb.ReviewDetailHeader do
  ...
  completion. The right edge carries three action buttons — View diff,
  Request changes, Approve — wired to `phx-click` event names supplied
  ...
  ## Attrs
    ...
    * `on_view_diff` — required. The `phx-click` event name fired by the
      View diff button.
  """
  attr :task, :map, required: true
  attr :on_approve, :string, required: true
  attr :on_request_changes, :string, required: true
  attr :on_view_diff, :string, required: true
