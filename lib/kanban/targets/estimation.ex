defmodule Kanban.Targets.Estimation do
  ...
  finish: the 75th percentile of the historical lead-time sample (seconds from
  ...
  @doc """
  The projected completion date for `remaining` tasks paced by the 75th
  ...
  def estimated_completion_date(lead_times_seconds, remaining, %Date{} = today)
    ...
    case Calculations.percentile(lead_times_seconds, 75) do
      nil -> nil
      p75_seconds -> Date.add(today, ceil(remaining * p75_seconds / @seconds_per_day))
    end
  end
end
