defmodule Kanban.Agents do
  ...
  boundary, that delegate the shared task set and counters back here:
    ...
    * `Kanban.Agents.Queries` — the scoped task fetch/projection
      (`fetch_tasks/1`, `fetch_target_bridged_tasks/1`) and the window/cap
      query helpers, re-exported through the delegates below.
  ...
  # --- Query (delegated to Kanban.Agents.Queries) ----------------------------
  # The scoped task fetch/projection and window/cap helpers were extracted into
  # `Kanban.Agents.Queries` (W1738). These delegates keep the `Kanban.Agents`
  # public API — the entry points the derivation modules, the LiveView, and the
  # DeliveryRollup bridge call — unchanged.

  @doc false
  defdelegate fetch_tasks(opts), to: Queries

  @doc false
  defdelegate fetch_target_bridged_tasks(opts), to: Queries

  @doc false
  defdelegate window_start_naive(days_back, timezone), to: Queries

  @doc false
  defdelegate time_range_days_back(range), to: Queries

  @doc false
  defdelegate within_fixed_window(tasks, days_back, timezone), to: Queries

  @doc false
  defdelegate within_time_range(tasks, range, timezone), to: Queries
